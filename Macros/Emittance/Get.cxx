...
void Get(const std::string& beam)
{
    std::cout << "Beam : " << beam << '\n';

    // Read data
    ROOT::EnableImplicitMT();
    ActRoot::DataManager datman {TString::Format("../../configs/data_%s.conf", beam.c_str()).Data(),
                                 ActRoot::ModeType::EFilter};
    ...
    auto def {df.Filter([](ActRoot::ModularData& m) { return m.Get("GATCONF") == 64; }, {"ModularData"})
                  ...
                  .Filter(
                      [](const Vector& first, const Vector& second)
                      {
                          ...
                          bool condB {second.front() >= 50}; // beams stop in TPC!!
                          return condA && condB;
                      },
    ...
    def.Snapshot("Emittance_Tree", TString::Format("./Outputs/emittance_%s.root", beam.c_str()),
