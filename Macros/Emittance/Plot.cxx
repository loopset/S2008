...
void Plot(const std::string& beam)
{
    std::cout << "Beam : " << beam << '\n';

    ROOT::EnableImplicitMT();
    ROOT::RDataFrame df {"Emittance_Tree", TString::Format("./Outputs/emittance_%s.root", beam.c_str())};
    // Compute angles
    ...
    auto file {std::make_unique<TFile>(TString::Format("./Outputs/histos_%s.root", beam.c_str()), "recreate")};
    hBegin->Write("hBegin");
    hEnd->Write("hEnd");
    hxz->Write("hTrajXZ");
    hyz->Write("hTrajYZ");
