...
double AZURECalc::operator()(const vector_r& p) const {
  ...
  for(EDataIterator data=localData->begin();data!=localData->end();data++) {
    ...
    //continue past segments to be summed
    if(data.segment()->GetSegmentKey()==1)  continue;
//    if(data.segment()->GetSegmentKey()==2)  continue;
    ...
    //now sum over previous segments
    if(data.segment()->GetSegmentKey()==2) {
      int pointIndex=data.point()-data.segment()->GetPoints().begin()+1;
      EPoint* previousPoint = (data.segment()-1)->GetPoint(pointIndex);
//      EPoint* previousPreviousPoint = (data.segment()-2)->GetPoint(pointIndex);
      fitCrossSection+=previousPoint->GetFitCrossSection();
//      fitCrossSection+=previousPreviousPoint->GetFitCrossSection();
    } 
